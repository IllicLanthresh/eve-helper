...
| `sde.test.js` | the client-side SDE, end to end and offline: a miniature Static Data Export assembled into a real zip and served from a range-capable local server, so the zip reader (end-of-directory probe, central directory, per-member ranges, `DecompressionStream`), the JSONL line reader (records split across chunks, a last line with no newline), and the full derivation all run on the production code path. Pins the schema the tools are written against — blueprint activities, `packagedVolume` from the SDE rather than a table, rig magnitudes from dogma with applicability from CCP's own modifier/filter records, the Thukker split, a rig with no market group left out, all three ore base-family rules, the compressed mapping — then breaks the fixture on purpose (moved Veldspar batch size, renumbered dogma attribute, a rig that no longer reads 4.20%, a server that refuses ranges) and checks each is **refused**. Finishes in a real page: the SDE status line, the Update button, the copy landing in IndexedDB, a reload reading it, and a newer upstream build being reported | yes |
