...
- **refined ISK/m³** — the variant's exact per-type reprocessing outputs (from the SDE via
  ...
  section-2 material prices and multiplied by *your* refine rate: the same skills and the
  same facility row (structure, rig, security band, implant) the mining plan uses. Ice
  products (isotopes, heavy water…) aren't in the section-2 grid, so their prices fetch
  on demand by type id.
